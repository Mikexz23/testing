(() => {
  ...
  function historicalProduct(){const record=historyData.find(r=>r.id===route.record);return record?{...record.p,amount:record.amount,rate:['2024-01-01','2024-06-15'].includes(record.date)?2.95:2.75,start:record.date,end:String(Number(record.date.slice(0,4))+1)+record.date.slice(4)}:selected();}
