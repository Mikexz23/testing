(() => {
  ...
  function home(){return `${statusbar()}<img class="home-image" src="02-首页.png" alt="招商银行首页：账户总览、转账、理财等服务"><button class="hotspot account-hotspot" data-action="account" aria-label="账户总览"></button><button class="hotspot profile-hotspot" data-go="login" aria-label="我的"></button>`;}
  function login(){return `${statusbar()}<img class="home-image" src="03-登陆.png" alt="我的：面容 ID 登录"><button class="hotspot face-hotspot" data-action="face" aria-label="使用面容 ID 登录"></button><button class="hotspot home-hotspot" data-go="home" aria-label="首页"></button><button class="hotspot options-hotspot" data-action="login-options" aria-label="更多登录选项"></button>`;}
