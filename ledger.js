...
window.createLedger=({header,icon,money,navigate,render,showModal,closeModal,choices,service,getRoute})=>{
  const entries=[
    ...
    {id:'june-transfer',month:'2024-06',day:'6.15',date:'2024-06-15',label:'转账-张建生(8896)',time:'11:36',amount:2230000,balance:2234306.15,type:'transfer',party:'张建生'},
    // The latest annual screenshot groups these under 7月, with a 6.15 day chip.
    {id:'recent-fund',month:'2026-07',day:'6.15',date:'2026-06-15',label:'蚂蚁（杭州）销售…',time:'14:29',amount:-640.47,balance:359.33,type:'fund'},
    {id:'recent-transfer',month:'2026-07',day:'6.15',date:'2026-06-15',label:'转账-张迅(1233)',time:'10:44',amount:1000,balance:1000,type:'transfer',party:'张迅'}
  ];
  ...
  function detail(){
    ...
    const info=[['交易卡号','6214********0813'],['交易时间',r.date+' '+r.time+(r.seconds?':'+r.seconds:'')],...(r.type==='transfer'?[['付款银行',r.party==='张建生'?'中国建设银行':'招商银行'],['付款账号',r.party==='张建生'?'6217********8896':'尾号1233'],['转账附言','转账']]:r.type==='fund'?[['交易渠道','<span class="alipay-mark">支</span> 支付宝']]:[]),['银行交易类型',r.type==='deposit'?'享定存开户起息':r.type==='fund'?'网联协议支付':'转账汇款']];
